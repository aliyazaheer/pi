    ...
    private void showErrorNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            notification = new Notification.Builder(this, ERROR_CHANNEL_ID)
                    ...
                    .setAutoCancel(true)
                    .setOngoing(true)
//                    .addAction(R.drawable.companylogo, "Stop Alarm", stopAlarmPendingIntent)
                    .build();
        } else {
    ...
    private void playAlarmSound() {
        Uri alarmSound = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_ALARM);
        mediaPlayer = MediaPlayer.create(getApplicationContext(), alarmSound); // Initialize the mediaPlayer
        if (mediaPlayer != null) {
            mediaPlayer.setLooping(true); // Loop the sound
            mediaPlayer.start();

            // Set OnCompletionListener to release resources when sound finishes
            mediaPlayer.setOnCompletionListener(mp -> {
                mp.release();
            });
        }
    }
    // Method to stop the sound when the notification is clicked
    void stopAlarmSound() {
        if (mediaPlayer != null && mediaPlayer.isPlaying()) {
            mediaPlayer.stop();
            mediaPlayer.release();
            mediaPlayer = null; // Set it to null to free up resources
        }
    }
